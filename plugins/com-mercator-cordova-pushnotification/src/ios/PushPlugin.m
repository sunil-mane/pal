...
- (void)openSettings:(CDVInvokedUrlCommand*)command {
    ...
    [self successWithMessage:@"settingsOpened"];
}
...
{
    if (self.callbackId == nil) {
        NSLog(@"Unexpected call to didFailToRegisterForUserNotificationSettingsWithError, ignoring: %@", errorMessage);
        return;
    }
    NSLog(@"Push Plugin register user notification settings failed");
    [self failWithMessage:errorMessage withError:nil];
}
